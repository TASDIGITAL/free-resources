...
// 1) Capture runtime errors (window.__errCapture) so we can diagnose the
//    page's debug overlay, and hide that overlay for visitors.
...
// Injected into every HTML page. Fetches /api/resources, pairs each
// resource card with its Airtable row (token-overlap matching, since card
// titles are display copy), then makes "Open" buttons open the link in a
// new tab once the visitor has passed the email gate (lead in localStorage).
// If the page later ships its own wiring (window.__resourceLinksWired),
// this script backs off.
const WIRE_SCRIPT = `(function () {
  if (window.__resourceLinksWired) return;
